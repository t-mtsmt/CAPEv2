        ...
        if blob:
            ...
            for i, cont in enumerate(configs):
                if cont in (b"\x00", b"\x01"):
                    ...
                    separator = next((x for x in (b"\n", b"|", b"\x1e", b"\xff\xff\xff\xff") if x in cont))
                    p_data["Control"] = []
                    for c2 in cont.strip(separator).split(separator):
...
if __name__ == "__main__":
    ...
    config = extract_config(open(sys.argv[1], "rb").read())
